...
ensureToolsAreInstalledProperly() {
arr=("$@")
for tool in "${arr[@]}";
  do :
    if [ ! -x "$(command -v "$tool")" ]; then
     echo "[-]" "$tool" "is required to run the script"
     exit 1
    fi
  done
}
...
if [ "$#" -ne 1 ]; then
    ...
    echo "Usage: sumrecon.sh example.com"
    exit 1
fi

url=$1
requiredTools=( "assetfinder" "amass" "sublist3r" "httprobe" "subjack" "waybackurls" "whatweb" "eyewitness" )
requiredOSTools=( "python3" "nmap" "curl" )

ensureToolsAreInstalledProperly "${requiredTools[@]}"
ensureToolsAreInstalledProperly "${requiredOSTools[@]}"
